...
def jobName = 'zeebe-script-worker-DISTRO-maven-deploy'
def repository = 'zeebe-script-worker'
...
JAR="zeebe-script-worker-${RELEASE_VERSION}.jar"
...
./github-release release --user zeebe-io --repo zeebe-script-worker --tag ${RELEASE_VERSION} --name "Zeebe Script Worker ${RELEASE_VERSION}" --description ""
./github-release upload --user zeebe-io --repo zeebe-script-worker --tag ${RELEASE_VERSION} --name "${JAR}" --file "${JAR}"
./github-release upload --user zeebe-io --repo zeebe-script-worker --tag ${RELEASE_VERSION} --name "${CHECKSUM}" --file "${CHECKSUM}"
'''
